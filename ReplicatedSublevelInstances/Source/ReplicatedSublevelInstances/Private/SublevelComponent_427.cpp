...
void USublevelComponent::AddToStreamingLevels(const FLevelStreamInstanceInfo &LevelInstanceInfo)
{
    ...
    if (World != nullptr)
    {
        ...
        if (!bAlreadyExists)
        {
			...
			// @todo: We can't use the non-deprecated call here, because
			// the NetConnection isn't available yet; we only have the
			// NetDriver. In future, if this call is actually removed, 
			// we might have to wholesale copy the NetworkRemapPath_local
			// implementation from the editor so that we can call based
			// on the WorldContext directly (which we can figure out
			// from GEngine and the World pointer).
